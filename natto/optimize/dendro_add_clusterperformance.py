...
def get_score(n1, n2, loader, trve):
    sampnum = 200 if debug else 1000
    d1 = loader(n1)
    d2 = loader(n2)
...
def get_params_100():
    ...
    loader = lambda x : load.load100(x,subsample=sampnum, remove_unlabeled = True)
    return dnames, -np.array(mtx), loader, trve
...
def get_params_punk():
    ...
    loader = lambda x: load.loadgruen_single(f"../data/punk/{x}", subsample=sampnum)
    return dnames, np.array(mtx), loader, trve
...
if __name__ == "__main__":
    ...
    dnames, mtx, loader, trve  = get_params_100()
    morder = np.argsort(mtx[task])
    result = []
    self = dnames[morder[0]]
    for did in morder[1:]: 
        other = dnames[did]
        result.append( get_score(self, other,loader,trve))
        gc.collect()
        if debug:
            break
