...
def cellDifferentiationPipeline(data, plot=True):
...
def optimalClusters(data, max_components=35, method='minBIC', plot=False):
	...
	for i in range(len(tempData)-1, -1, -1):
		...
		models = [GaussianMixture(n_components=n, covariance_type='full', random_state=1337).fit(dataStacked) 
			for n in range(1, clusterMax+1)]
    	bic = [ m.bic(d) for m in models ]
    	bestBICIndex = bestBIC(method, clusterMax, bic, plot)

	    print(f"Best num Clusters is {nComponents[bestBICIndex]}\n")
	    labels[i] = models[bestBICIndex].predict(d)
	    clusterMax = bestBICIndex

	return labels

def bestBIC(method, clusterMax, bic, plot, clusterMax):
	if method == 'kneed':
        ...
        bestBICIndex = np.argmin([b + (np.log(i+1)*(len(tempData))) for i,b in enumerate(bic)])
