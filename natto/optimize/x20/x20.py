...
def preprocess(types = 5, samples =4, repeats =7):
    n = input.get71names()
    datasets = []
    random.seed(43)
...
if what == 'preprocess':
    ...
    res = preprocess(3,3,3) # => 9x9x3 matrix :)
...
def calc_mp20(method_label, fname='data.dmp', shape=(20,20,7)):
        ...
        tools.dumpfile(m.median(axis=2),f'{saveas}.dmp')
...
def plot(methodlabel):
        ...
        pred = AgglomerativeClustering(n_clusters=5,affinity='precomputed',linkage='single').fit_predict(m)
...
if what == 'plot':
    #tools.xmap(plot,methodlabels[1:],32)
