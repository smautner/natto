...
def get57names():
    ...
    # datasets have more than 5000 cells and there are at least 4
    # examples per cluster, there are 8 clusters
...
def get40names():
    # refinement of 57 as tomor and testi have many cells with few reads
    n = get57names()
    f = lambda st: not st.startswith('Tumor') and not st.startswith('Testi')
    return [nn for nn in n if f(nn)]
...
def get71names():
    #Counter({'Testi': 13, 'Colon': 10, 'Prost': 10, 'Tcell': 10, 'Bonem': 8, 'Cordb': 6, 'Kapos': 6, 'Kidne': 4, 'Ovari': 4})
